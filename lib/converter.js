                    ...
                    case "CallExpression" :
                        ...
                        if(node.callee.property.name === "forEach") {
                            ...
                            if(node.arguments[0].params.length > 1) {
                                ...
                                {
                                    console.warn("Only one argument is supported in current version\n")
                                ...
                                let firstArg = node.arguments[0].params[0].name 

                                // 4 -> fetch actual code inside forEach
                                let { start , end } = node.arguments[0].body
                                let codeInsideForEach = this.code.substr(start+1, end-start-2)
